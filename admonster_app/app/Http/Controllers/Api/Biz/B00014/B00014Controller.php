...
class B00014Controller extends Controller
{
    /**
     *  AG業務件数レポート（長春対応分）
     * @param Request $req
    ...
    public function agRegisterReport(Request $req)
    {
        try {
            ...
            $started_at = parse_utc_string_to_user_timezone_date($req->started_at)->format('Y/m/d');//期間From
            $finished_at = parse_utc_string_to_user_timezone_date($req->finished_at)->format('Y/m/d');//期間To
    ...
    private function makeReport(string $started_at, string $finished_at): array
    {
        ...
        // テンプレートファイルの読み込み
        $template_file_path = storage_path('biz/b00014/template.xlsx');
        ...
        foreach ($result as $data) {
            ...
            if (!empty($data->approvals_update_at)) {
                $approvals_date = Carbon::parse($data->approvals_update_at)->format('YmdHis');
            }
        ...
        $file_name = 'AG業務件数レポート（長春）_' . $user_timezone_started_at->format('Ymd') . '-' . $user_timezone_finished_at->format('Ymd') . '.xlsx';
