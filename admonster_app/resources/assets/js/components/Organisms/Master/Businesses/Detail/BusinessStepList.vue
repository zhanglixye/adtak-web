    ...
    computed: {
        headers (){
            return  [
                ...
                { text: this.$t('businesses.step_type'), value: 'step_type', align: 'center', sortable: false},
                { text: this.$t('businesses.description'), value: 'step_description', align: 'center', sortable: false},
                { text: this.$t('businesses.deadline'), value: 'deadline_limit', align: 'center', sortable: false},
                { text: this.$t('businesses.candidate'), value: 'work_user_ids', align: 'center', sortable: false}
            ]
        },
    },
    methods: {
        ...
        showTimeUnit (time_unit) {
            if (time_unit ===  _const.TIME_UNIT.MINUTE){
                return this.$t('common.datetime.minute')
            } else if (time_unit ===  _const.TIME_UNIT.HOUR){
                return this.$t('common.datetime.hour.interval')
            } else if (time_unit ===  _const.TIME_UNIT.DAY){
                return this.$t('common.datetime.day')
            }
        },
        showStepType (step_type) {
            if (step_type === _const.STEP_TYPE.INPUT){
                return this.$t('businesses.detail.step_type.input')
            } else if (step_type === _const.STEP_TYPE.APPROVAL){
                return this.$t('businesses.detail.step_type.approval')
            }
        },
