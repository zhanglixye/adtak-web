...
export default {
    ...
    created () {
        eventHub.$on('commitHeaderCustomData', function(data) {
            ...
            // TODO keyの命名規約
            localStorage.setItem('allocationHeaders', JSON.stringify(data));
        })
        eventHub.$on('resetHeaderCustomData', function() {
            ...
            try {
                ...
                localStorage.setItem('allocationHeaders', JSON.stringify(localHeadersdata));
            } catch (e) {
                console.log(e)
                localStorage.removeItem('allocationHeaders');
        ...
        // TODO keyの命名規約
        if (localStorage.getItem('allocationHeaders')) {
            try {

                let localHeadersdata = JSON.parse(localStorage.getItem('allocationHeaders'));
    ...
    methods: {
        ...
        displayStatusName(status) {
            ...
            if (status === null){
                // 未割振
                name = Vue.i18n.translate('allocations.list.status.none')
            } else {
                // 割振済
                name = Vue.i18n.translate('allocations.list.status.done')
            }
            return name
        },
        ...
        displayAllocationMethod(isAuto, allocationMethod) {
            ...
            if (!isAuto){
                return method = Vue.i18n.translate('allocations.list.is_auto.false')
            }

            method = Vue.i18n.translate('allocations.list.is_auto.true')+'('

            switch (allocationMethod) {
            case _const.ALLOCATION_METHOD.EQUALITY:
                method = method + Vue.i18n.translate('allocations.list.method.equality')+')'
                break;
            case _const.ALLOCATION_METHOD.BUSINESS_VOLUME:
                method = method + Vue.i18n.translate('allocations.list.method.business_volume')+')'
                break;
            case _const.ALLOCATION_METHOD.LEARNING_LEVEL:
                method = method + Vue.i18n.translate('allocations.list.method.learning_level')+')'
                break;
            }
            return method
        },
        searchRequestListAsync (params = this.searchParams) {
            ...
            axios.post('/api/allocations/index', searchParams)
                .then((res) =>{
                // 検索条件をstoreに保存
