                        ...
                        <v-layout row wrap>
                            ...
                            <v-flex sm6 md4>
                                <v-layout column wrap>
                                    <v-flex md12 pr-3>
                                        <v-text-field
                                            v-model="searchParams.deliverer"
                                            prepend-icon="assignment_ind"
                                            hide-details
                                            :label="$t('list.search_condition.deliverer')"
                                            :placeholder="$t('list.search_condition.partial_search')"
                                            @keyup.enter="search"
                                        ></v-text-field>
                                    </v-flex>
...
const DELIVERY_STATUSES = [
    { text: Vue.i18n.translate('deliveries.list.status.none'), value: _const.DELIVERY_STATUS.NONE },
    { text: Vue.i18n.translate('deliveries.list.status.done'), value: _const.DELIVERY_STATUS.DONE },
    { text: Vue.i18n.translate('deliveries.list.status.before'), value: _const.DELIVERY_STATUS.SCHEDULED },
]
...
export default {
    ...
    data: () => ({
        ...
        searchParams: {
            ...
            delivery_status: store.state.searchParams.delivery_status,
            deliverer: store.state.searchParams.deliverer,
    ...
    computed: {
        dateTypeLabel () {
            ...
            } else if (this.searchParams.date_type === _const.DATE_TYPE.SCHEDULED) {
                return this.$t('list.search_condition.delivered')
            } else {
    ...
    methods: {
        toggleDateType () {
            ...
            } else if (this.searchParams.date_type === _const.DATE_TYPE.DEADLINE) {
                this.searchParams.date_type = _const.DATE_TYPE.SCHEDULED
            } else {
        ...
        setSearchParams () {
            ...
            this.searchParams.delivery_status = store.state.searchParams.delivery_status
            this.searchParams.deliverer = store.state.searchParams.deliverer
