                ...
                <v-layout row wrap>
                    <v-flex xs12 mb-2>
                        <h5 class="headline ma-0">
                            {{ $t('biz.wf_gaisan_syusei.assort_mail.h1') }}
                        </h5>
                    </v-flex>
                    <v-flex xs12>
...
import IrregularModal from '../../../Organisms/Biz/WfGaisanSyusei/IrregularModal.vue'
import MailToAgency from '../../../Organisms/Biz/WfGaisanSyusei/AssortMail/MailToAgency.vue'
import Steps from '../../../Organisms/Biz/WfGaisanSyusei/AssortMail/Steps.vue'
...
export default {
    ...
    methods:{
        getData: function(type) {
            this.loading = true
            axios.get('/api/biz/wf_gaisan_syusei/assort_mail/' + this.requestWorkId + '/' + this.taskId + '/create')
                .then((res) => {
                    if (type == 'initial') {
                        // 初回ロード時
