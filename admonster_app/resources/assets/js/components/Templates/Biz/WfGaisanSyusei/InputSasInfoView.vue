                ...
                <v-layout row wrap>
                    <v-flex xs12 mb-2>
                        <h5 class="headline ma-0">
                            {{ $t('biz.wf_gaisan_syusei.input_sas_info.h1') }}
                        </h5>
                    </v-flex>
                    <v-flex xs12>
...
import MasterSheet from '../../../Organisms/Biz/WfGaisanSyusei/InputSasInfo/MasterSheet.vue'
import Steps from '../../../Organisms/Biz/WfGaisanSyusei/InputSasInfo/Steps.vue'
...
export default {
    ...
    methods:{
        getData: function(type) {
            this.loading = true
            axios.get('/api/biz/wf_gaisan_syusei/input_sas_info/' + this.requestWorkId + '/' + this.taskId + '/create')
                .then((res) => {
                    if (type == 'initial') {
                        // 初回ロード時
