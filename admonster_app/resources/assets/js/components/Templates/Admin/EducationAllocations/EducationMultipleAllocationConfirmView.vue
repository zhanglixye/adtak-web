<template>
    <v-app id="inspire">
        ...
        <app-header
            :title="businessName"
            :subtitle="$t('education_allocations.collective_assignment_confirm')"
        ></app-header>
                        ...
                        <v-layout row wrap>
                            <v-flex xs3>
                                <v-card class="allocation-condition text-xs-center">
                                    <v-card-text class="px-0">{{ $t('education_allocations.allocation_list.number_of_eligible_projects') }}</v-card-text>
                            ...
                            <v-flex xs3>
                                <v-card class="allocation-condition text-xs-center">
                                    <v-card-text class="px-0">{{ $t('education_allocations.number_of_occurrences') }}
                                        <v-tooltip top>
                                            <span slot="activator"><v-icon small>contact_support</v-icon></span>
                                            <span class="text-xs-center">{{ $t('education_allocations.target_project_parallel') }}<br>{{ $t('education_allocations.unworked_only') }}</span>
                                        </v-tooltip>
                                    </v-card-text>
                                    <v-divider class="ma-0"></v-divider>
                                    <v-card-text class="px-0">
                            ...
                            <v-flex xs3>
                                <v-card class="allocation-condition text-xs-center">
                                    <v-card-text class="px-0">{{ $t('education_allocations.concurrency') }}</v-card-text>
                                    <v-divider class="ma-0"></v-divider>
                                    <v-card-text class="px-0">
                                        <template>
                                            <v-layout pr-2 pl-2 align-center>
                                                <v-flex>
                                                    <span>{{ $t('education_allocations.allocation_list.per_case') }}</span><span class="display-1 font-weight-bold pl-2 pr-2">{{ parallel }}</span><span>{{ $t('education_allocations.allocation_list.assign_people') }}</span>
                                                </v-flex>
                                            </v-layout>
                                        </template>
                                    </v-card-text>
                            ...
                            <v-flex xs3>
                                <v-card class="allocation-condition text-xs-center">
                                    ...
                                    <v-divider class="ma-0"></v-divider>
                                    <v-card-text class="px-0">
                                        <span v-if="!evennessIndividual" class="headline font-weight-bold mr-1">{{ $t('education_allocations.evenness_type.even') }}</span>
                                        <span v-else class="headline font-weight-bold mr-1">{{ $t('education_allocations.evenness_type.individual') }}
                    ...
                    <v-toolbar tabs color="grey darken-1">
                        ...
                        >
                            ...
                            <v-tab href="#tabs-request-works" class="primary--text">
                                ...
                                <span class="tab-title ml-1">{{ $t('education_allocations.by_project') }}</span>
                            </v-tab>
                            <v-tab href="#tabs-operators" class="primary--text">
                                <v-icon color="grey lighten-3">assignment_ind</v-icon>
                                <span class="tab-title ml-1">{{ $t('education_allocations.by_rep') }}</span>
                            </v-tab>
                        </v-tabs>
                    </v-toolbar>
...
export default {
    ...
    computed: {
        parallelHint () {
            return this.$t('education_allocations.allocation_list.default_value') + this.parallel
        },
        userImageSrc () {
    ...
    methods: {
        getInitData (param) {
            this.loading = true
            axios.post('/api/education/multiple_allocations/confirm',  param)
                .then((res) => {
        ...
        save: function () {
            ...
            axios.post('/api/education/multiple_allocations/store',  params)
                .then((res) => {
